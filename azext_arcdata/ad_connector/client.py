...
def beget(az_cli, kwargs):
    """Client factory"""
    return ActiveDirectoryConnectorClientMixin(az_cli, kwargs)


def beget_no_namespace(az_cli, kwargs):
    """Client factory - no check on namespace"""
    return ActiveDirectoryConnectorClientMixin(
        az_cli, kwargs, check_namespace=False
    )


class ActiveDirectoryConnectorClientMixin(CliClient):
    def __init__(self, az_cli, kwargs, check_namespace=True):
        super(ActiveDirectoryConnectorClientMixin, self).__init__(
