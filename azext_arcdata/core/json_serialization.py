...
def hydrate_from_dictionary(instance, obj):
    ...
    def mapVal(destination, key):
        try:
            ...
            jsonType = _.get(
                type(destination),
                "{0}.fget.__wrapped__._jsonType".format(key),
                False,
            )
            isList = _.get(
                type(destination),
                "{0}.fget.__wrapped__._isList".format(key),
                False,
            )
            if jsonType:
                if isList:
...
class jsonProperty(property):
    ...
    def __set__(self, obj, value):
        ...
        if changed_properties is not None:
            changed_properties.add(_.get(self, "fset.__name__"))
...
def jsonSerializable(cls):
    ...
    def _changed_keys(self):
        ...
        indirect_changed_keys = _.filter_(
            unchanged_keys,
            lambda k: len(_.result(self, "{0}._changed_keys".format(k), []))
            > 0,
        )
